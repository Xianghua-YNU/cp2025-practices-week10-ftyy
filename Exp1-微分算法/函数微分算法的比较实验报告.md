...
## 四、实验结果与分析
### 4.1 导数计算结果对比
<img width="375" alt="截屏2025-04-30 11 41 29" src="https://github.com/user-attachments/assets/d34bc143-90dc-4a11-9a6d-39d129a0433b" />


### 4.2 误差分析 
#### 4.2.1 中心差分法误差分析
<img width="356" alt="截屏2025-04-30 11 42 15" src="https://github.com/user-attachments/assets/c139cd57-0cc3-40e5-8290-2e6072657122" />

#### 4.2.2 Richardson外推法误差分析
<img width="356" alt="截屏2025-04-30 11 42 15" src="https://github.com/user-attachments/assets/33676db2-f55d-449a-a4c3-59dbc1be9658" />


## 五、实验讨论
### 5.1 两种方法的优缺点分析
1. 中心差分法
   - 优点：（1）实现简单:​​只需计算相邻两点的函数值，公式直观，代码易于编写；（2）低计算成本​​：每个点仅需两次函数调用，适合大规模数据快速计算。
   - 缺点：（1）边界限制​​：无法处理首尾点，需额外方法（如单边差分）补充；（2）步长敏感​​：需权衡截断误差与舍入误差，最优步长依赖函数特性。
2. Richardson外推法
   - 优点：（1）高阶精度​​：通过外推消除低阶误差项，精度可达 O(h**2m)（m 为外推次数）；（2）​​收敛加速​​：显著提升收敛速度，适合高精度需求场景。
   - 缺点：（1）计算复杂度高​​：需多次计算不同步长的差分，外推阶数 m 增加时，计算量呈O(m**2） 增长；（2）数值不稳定性​​：高阶外推可能因分母 4**m−1 放大舍入误差，导致结果震荡。
